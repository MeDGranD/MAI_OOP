...
Array<T>::Array(): cap(1), sz(0){
    this->arr = std::shared_ptr<T[]>(reinterpret_cast<T*>(new char[sizeof(T)]));
}
...
Array<T>::Array(const std::initializer_list<T>& _list): cap(_list.size()), sz(_list.size()){
    ...
    std::shared_ptr<T[]> tempArray = std::shared_ptr<T[]>(reinterpret_cast<T*>(new char[sizeof(T) * _list.size()]));

    try{
        std::uninitialized_copy(_list.begin(), _list.end(), tempArray.get());
    }
    catch(...){
        tempArray.reset(new T[1]);
        throw;
    }
...
Array<T>::Array(const Array<T>& _other) noexcept : Array() {
    ...
    for(size_t index = 0; index < this->sz; ++index){
        new(this->arr.get() + index) T(_other[index]);
    }
}
...
Array<T>& Array<T>::operator=(const Array<T>& other){
    ...
    this->arr.reset(reinterpret_cast<T*>(new char[sizeof(T) * other.capacity()]));
...
Array<T>& Array<T>::operator=(Array<T>&& temp){
    ...
    this->arr = temp.arr;
    temp.arr = std::shared_ptr<T[]>(new T[1]);
...
void Array<T>::resize(const size_t _newSize, const T& _data){
    ...
    if(_newSize < this->sz){
        ...
        for(size_t index = this->sz; index < _newSize; ++index){
            new(this->arr.get() + index) T(_data);
        }
    }
    this->sz = _newSize;
}
...
void Array<T>::reserve(size_t _newCap){
    std::shared_ptr<T[]> tempArray = std::shared_ptr<T[]>(reinterpret_cast<T*>(new char[sizeof(T) * _newCap]));

    try{
        if(_newCap > this->cap){
            std::uninitialized_copy(this->arr.get(), this->arr.get() + this->cap, tempArray.get());
        }
        else{
            std::uninitialized_copy(this->arr.get(), this->arr.get() + _newCap, tempArray.get());
        }
    }
    catch(...){
        this->arr.reset(new T[1]);
        throw;
    }
